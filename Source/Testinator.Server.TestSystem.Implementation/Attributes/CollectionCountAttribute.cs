...
{
    internal class CollectionCountAttribute : BaseEditorAttribute
    {
        public int Max { get; private set; }
        public int Min { get; private set; }

        public CollectionCountAttribute(int max, int min = 0)
        {
            Initialize(max, min);
        }

        public CollectionCountAttribute(int max, int fromVersion, int min = 0) : base(fromVersion)
        {
            Initialize(max, min);
        }
