...
namespace Testinator.Server.Network
{
    /// <summary>
    ...
    public abstract class ServerBase : IServer
    {
        ...
        private void Send(Socket Target, DataPackage Data)
        {
            if (Target == null)
            {
                mLogger.Log("Could not send the data. Target socket was null");
                throw new ServerException(LocalizationResource.CouldNotSendData, "Target socket was null");
            }

            if (Data == null)
            {
                mLogger.Log("Could not send the data as it was null");
            ...
            {
                mLogger.Log($"Failed to send the data. Error: {ex.Message}");
                throw new ServerException("Failed to send the data", ex);
            }

            mLogger.Log("Data sent successfully");
        }
        ...
        public void Start()
        {
            ...
            catch (Exception ex)
            {
                ...
                mLogger.Log($"Failed to start the server. Error: {ex.Message}");

                throw new ServerException(LocalizationResource.CouldNotStartServer, ex);
            }

            mLogger.Log("Server has been started correctly");
        ...
        public void ShutDown()
        {
            ...
            mLogger.Log("Attempting to shut the server down...");
            ...
            mLogger.Log("Sending disconnection requests and closing sockets..");
            ...
            {
                mLogger.Log($"Server shutdown error: {ex.Message}");
                
                // Clear the clients 
                mClients.Clear();
            }

            mLogger.Log("All connected sockets closed\nClosing server socket...");

            // Also close the server socket to prevent from accepting new clients
            mServerSocket.Close();

            mLogger.Log("Server socket closed");
        ...
        public void Send(ClientModel TargetClient, DataPackage Data)
        {
            ...
            catch (ServerException ex)
            {
                ...
                mLogger.Log(ex.Message + ex.Reason);
            }
        }
        ...
        private void AcceptCallback(IAsyncResult ar)
        {
            ...
            mLogger.Log("Server: New connection. Trying to get the socket...");
            ...
            {
                mLogger.Log("Failed to extract the socket");
                return;
            }
            ...
            mLogger.Log("Socket connected succesfully.");
        ...
        private void ReceiveCallback(IAsyncResult ar)
        {
            ...
            if (DataPackageDescriptor.TryConvertToObj<DataPackage>(recBuf, out var ReceivedPackage))
            {
                ...
                mLogger.Log("Skipping data package. Binary conversion failed.");
        ...
        /// Default constructor
        /// </summary>
        public ServerBase(ILogFactory logger)
        {
            // Inject DI services
            mLogger = logger;
